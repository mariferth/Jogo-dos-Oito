...
def verificaIgual(abertos, fechados, estado):
    for i in range(len(abertos)):
        if (estado == abertos[i]):
            return 1
    for i in range(len(fechados)):
        if (estado == fechados[i]):
            return 1   
    return 0   
...
def bfs(tab):
    ...
    while abertos != None:
        ...
        if x == obj:
            ...
            for i in range(len(fechados)):
                print(i+1)
                imprime_tabuleiro(fechados[i])
            print('Quantidade de passos até encontrar o Objetivo: ', len(fechados)-1)
            ...
            if pc != None: 
                if verificaIgual(abertos, fechados, pc) == 0: abertos.append(pc)
            if pb != None: 
                if verificaIgual(abertos, fechados, pb) == 0: abertos.append(pb)
            if pe != None:
                if verificaIgual(abertos, fechados, pe) == 0: abertos.append(pe)
            if pd != None:
                if verificaIgual(abertos, fechados, pd) == 0: abertos.append(pd)
    return 'FALHA :('
###############################################################################################
...
def verificaIgual_h(abertos, fechados, est, f, g, h):
    for i in range(len(abertos)):
...
def A_Estrela_h1(tab):
    ...
    while abertos != None:
        ...
        if x.matriz == obj:
            fechados.append(x)
            print('CAMINHO:')
            for i in range(0, len(fechados)):
                print(i+1)
                imprime_tabuleiro(fechados[i].matriz)
            print('Quantidade de passos até encontrar o Objetivo: ', len(fechados)-1)
            print('Quantidade de estados abertos: ', len(abertos))
            print('Quantidade de estados fechados: ', len(fechados))
            return('SUCESSO!')
            ...
            pd = paraDireita(aux)
            if pc != None:  
                if verificaIgual_h(abertos, fechados, pc, h1(pc) + g, g, h1(pc)) == 0:
                    estado2 = Estado(pc, h1(pc) + g, g, h1(pc), x)
                    abertos.append(estado2)
            if pb != None: 
                if verificaIgual_h(abertos, fechados, pb, h1(pb) + g, g, h1(pb)) == 0: 
                    estado3 = Estado(pb, h1(pb) + g, g, h1(pb), x)
                    abertos.append(estado3)
            if pe != None: 
                if verificaIgual_h(abertos, fechados, pe, h1(pe) + g, g, h1(pe)) == 0:
                    estado4 = Estado(pe, h1(pe) + g, g, h1(pe), x)
                    abertos.append(estado4)
            if pd != None:
                if verificaIgual_h(abertos, fechados, pd, h1(pd) + g, g, h1(pd)) == 0:
...
def A_Estrela_h3(tab):
    ...
    while abertos != None:
        ...
        if x.matriz == obj:
            ...
            pd = paraDireita(aux)
            if pc != None:  
                if verificaIgual_h(abertos, fechados, pc, h3(pc) + g, g, h3(pc)) == 0:
                    estado2 = Estado(pc, h3(pc) + g, g, h3(pc), x)
                    abertos.append(estado2)
            if pb != None: 
                if verificaIgual_h(abertos, fechados, pb, h3(pb) + g, g, h3(pb)) == 0: 
                    estado3 = Estado(pb, h3(pb) + g, g, h3(pb), x)
                    abertos.append(estado3)
            if pe != None: 
                if verificaIgual_h(abertos, fechados, pe, h3(pe) + g, g, h3(pe)) == 0:
                    estado4 = Estado(pe, h3(pe) + g, g, h3(pe), x)
                    abertos.append(estado4)
            if pd != None:
                if verificaIgual_h(abertos, fechados, pd, h3(pd) + g, g, h3(pd)) == 0:
